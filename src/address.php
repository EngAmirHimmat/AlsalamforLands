...
<!---menu-->
	...
	<div class="menuBar" >
		<ul>
			...
			<li><a  href="#" class="select">العنوانين</a></li>
...
<!---menu2-->
	...
	<div class="menuBar2" >
		<ul>
			<li><a href="AddNewDistric.php">إضافة عنوان جديد</a></li>
			<div  class="c"></div>
		</ul>
		<div></div>
	</div>
...
if ($result=mysqli_query($conn_link,$query)) 
        {
          while ($row=mysqli_fetch_assoc($result)) 
          {
				?>
	<tr>
		<td><?php echo $row['dis_num'];?></td>
		<td><?php echo $row['dis_name'];?></td>
		<td><?php echo $local[$row['local_num']];?></td>
		<td><?php echo $ad_unit[$row['adm_num']];?></td>
		<td>
			<form method="post" action="delete.php">
				<input type="hidden" name="id" value="<?php echo $row['dis_num'];?>">
				<button type="submit" name="delete" value="distric">حذف</button>	
			</form>	
		</td>
	</tr>
<?php
